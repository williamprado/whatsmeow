...
func TestBuildCarouselMessage(t *testing.T) {
	msg := BuildCarouselMessage("Check these out", []CarouselCard{
		{
			Body:   "Card 1",
			Footer: "f1",
	...
	if im.GetBody().GetText() != "Check these out" {
		t.Errorf("intro body = %q, want %q", im.GetBody().GetText(), "Check these out")
	}
	carousel := im.GetCarouselMessage()
	if carousel == nil {
	...
	if cards[0].GetBody().GetText() != "Card 1" {
		t.Errorf("card[0] body = %q, want %q", cards[0].GetBody().GetText(), "Card 1")
	}
	if cards[0].GetFooter().GetText() != "f1" {
		t.Errorf("card[0] footer = %q, want %q", cards[0].GetFooter().GetText(), "f1")
	}
	if cards[1].GetFooter() != nil {
		t.Error("card[1] footer should be nil")
	}
